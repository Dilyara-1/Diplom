  ...
  return (
    <div>
      <div className="my-4 stats inline-grid md:flex  shadow stats-vertical   md:stats-horizontal">
        <div className="stat">
          <div className="stat-title">Продажи</div>
          <div className="stat-value text-primary">
            {formatNumber(summary.ordersPrice)} ₽
          </div>
          <div className="stat-desc">
            <Link href="/admin/orders">Посмотреть продажи</Link>
          </div>
        </div>
        <div className="stat">
          <div className="stat-title"> Заказы</div>
          <div className="stat-value text-primary">{summary.ordersCount}</div>
          <div className="stat-desc">
            <Link href="/admin/orders">Посмотреть заказы</Link>
          </div>
        </div>
        <div className="stat">
          <div className="stat-title">Товары</div>
          <div className="stat-value text-primary">{summary.productsCount}</div>
          <div className="stat-desc">
            <Link href="/admin/products">Посмотреть товары</Link>
          </div>
        </div>
        <div className="stat">
          <div className="stat-title">Пользователи</div>
          <div className="stat-value text-primary">{summary.usersCount}</div>
          <div className="stat-desc">
            <Link href="/admin/users">Посмотреть пользователей</Link>
          </div>
        </div>
      </div>
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          <h2 className="text-xl py-2">Отчет о продажах</h2>
          <Line data={salesData} />
        </div>
        <div>
          <h2 className="text-xl py-2">Отчет о заказах</h2>
          <Line data={ordersData} />
        </div>
      </div>
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          <h2 className="text-xl py-2">Отчет о товарах</h2>
          <div className="flex items-center justify-center h-80 w-96 ">
            <Doughnut data={productsData} />
          </div>
        </div>
        <div>
          <h2 className="text-xl py-2">Отчет о пользователях</h2>
          <Bar data={usersData} />
        </div>
      </div>
    </div>
